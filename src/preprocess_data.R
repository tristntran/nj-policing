...
department_df <- read_csv("data/UOF_BY_DEPARTMENTS.csv") %>% 
  ...
  rename("pct_compliance_hold" = "pct_complaince_hold")
...
department_df$period <- "2012-2016"
...
department_df2 <- officer_df %>% group_by(pd_dept) %>% 
  ...
  mutate(pct_compliance_hold = count_compliance_hold/count_incedents * 100,
  ...
  select(pd_dept, pct_compliance_hold, pct_hands_fists,
         pct_pepper_spray, pct_baton, pct_take_down,
         pct_leg_strikes, pct_deadly_force)
department_df2$period <- "2020-2021"
